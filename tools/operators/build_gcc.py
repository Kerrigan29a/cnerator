...
CFLAGS = '-Wall -Wpedantic -Wextra -Wno-unused -Werror -m32 -O0 -I"C:\\MinGW\\include" -I"C:\\MinGW\\lib\\gcc\\mingw32\\9.2.0\\include" -I"C:\\MinGW\\lib\\gcc\\mingw32\\9.2.0\\include-fixed" -I"C:\\MinGW\\var\\lib\\wsl\\include"'
lock = Lock()
...
def compile(f):
    ...
    while True:
        run(f'("{ACTIVATE_BAT}" && gcc {CFLAGS} -c -o {obj} {c}) > {compile_out} 2> {compile_err}', shell=True)
        ...
        run(f'python commenter.py {compile_err} -p config_gcc.json -b -o -e utf8 -E utf8 -c "// (GCC)" > {comment_out} 2> {comment_err}', shell=True)
