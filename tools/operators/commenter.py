...
def run(args):
    ...
    if not commented_lines:
        print("No lines to comment. Problably you should check the patterns file.")
        return 0
            ...
            for line in f:
                ...
                if t:
                    if t == "comment":
                        ...
                        print("Commented {}:{} -> {}".format(file, line, note))
                    continue
                if args.batch:
                    continue
...
def expand_comments(patterns, commented_lines):
    ...
    for file, lines in commented_lines.items():
        for line in lines:
            line = int(line, 10)
            for pattern, _range in patterns.comment_range.items():
                ...
                if m:
                    for new_line in range(line + _range.begin, line + _range.end + 1):
                        ...
                        print("Commented (expansion) {}:{}".format(file, new_line))
...
def parse_action(action_line):
    ...
    parser_ignore.add_argument("-p", "--pattern", help="Ignore all lines with this pattern")
    ...
    parser_comment.add_argument("pattern", help="Comment all lines with this pattern")
...
def search_all_patterns(line, patterns, search_comments_only):
    ...
    for pattern, groups in patterns.comment.items():
