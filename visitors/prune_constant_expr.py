...
"""
This visitor replaces all constant expression with a nliteral.
"""
...
def _(program: ast.Program, **kwargs) -> ast.Program:
    print_if_verbose()
    print_if_verbose("*" * 80)
    print_if_verbose("* PRUNE CONSTANT EXPRESSIONS")
    print_if_verbose("*" * 80)
...
def _(node: ast.Return, program: ast.Program, function: ast.Function, **kwargs) -> ast.Return:
...
def _(node, program: ast.Program, function: ast.Function, **kwargs) -> ast.ASTNode:
    node.children = [visit(ch, program, function, **kwargs) if isinstance(ch, ast.ASTNode) else ch for ch in node.children]
    if isinstance(node.type, (ast.Struct, ast.Array, ast.Pointer)):
        return node
    if all(isinstance(ch, ast.Literal) for ch in node.children):
        print_if_verbose("Pruning constant expression: " + node.to_str())
        literal = node.type.generate_literal(from_declaration=False)
        literal.value = f"/* PRUNED CONSTANT EXPR */ {literal.value}"
        return literal
    return node
